...
相比DQN,只做了两处修改,并加了Different from DQN的注释.
"""
...
class Double_DQN:
    ...
    def learn(self):
        ...
        q_next, q_eval = self.sess.run(
            ...
            feed_dict={
                self.s_: batch_memory[:, -self.n_features:],  # S_在target网络的不同action的Q(S_,a)值
                self.s: batch_memory[:, :self.n_features],  # S在eval的不同action的Q(S,a)值
            })
        #Different from DQN
        q_eval_again = self.sess.run(
            self.q_eval,
            feed_dict={
                self.s: batch_memory[:, -self.n_features:],  # S_在eval网络的不同action的Q(S_,a)值
            })    
        #---------------------------------------------------------------------------------------
        ...
        #这里就是生成了0到batch_size的列表
        batch_index = np.arange(self.batch_size, dtype=np.int32)
        #获取样本中action的下标 ,batch_memory 的格式是[s,s,a,r,s_,s_] ,由于state是两维的,所以它占两个位置
        eval_act_index = batch_memory[:, self.n_features].astype(int)
        #获取reward, 1*n
        reward = batch_memory[:, self.n_features + 1]
        #把q_target中对应的Q(s,a)改成target值,r+gamma*maxQ(s,a). ,这样在求loss时,用q_target - q_eval,除了最大的a那里有值,其它位置都是0.这个值就类似label和预测值之间的差值
        #Different from DQN        
        q_target[batch_index, eval_act_index] = reward + self.gamma * q_next[batch_index,np.argmax(q_eval_again, axis=1)]
