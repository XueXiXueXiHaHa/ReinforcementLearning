...
class DDPG(object):
    ...
    def update_target_network(self):
        #经过n此学习后,更新target网络参数
        """
        if self.a_replace_counter % REPLACE_ITER_A == 0:
            self.sess.run([tf.assign(t, tf.multiply(e, TAU) + tf.multiply(t, 1 - TAU)) for t, e in zip(self.at_params, self.ae_params)])
        if self.c_replace_counter % REPLACE_ITER_C == 0:
            self.sess.run([tf.assign(t, tf.multiply(e, TAU) + tf.multiply(t, 1 - TAU)) for t, e in zip(self.ct_params, self.ce_params)])
        """
        if self.a_replace_counter % REPLACE_ITER_A == 0:
